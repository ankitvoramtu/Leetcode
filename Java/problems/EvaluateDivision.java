...
public class EvaluateDivision {
	public double[] calcEquation(String[][] equations, double[] values,
		...
		for (int i = 0; i < queries.length; i++) {
			...
			result[i] = dfs(query[0], query[1], pairs, valuesPair,
	...
	private double dfs(String start, String end,
			...
			HashMap<String, ArrayList<Double>> values, HashSet<String> set,
			double value) {
		...
		for (int i = 0; i < strList.size(); i++) {
			tmp = dfs(strList.get(i), end, pairs, values, set, value
					* valueList.get(i));
			if (tmp != 0.0) {
				break;
			}
