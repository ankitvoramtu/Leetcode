...
public class BestTimeToBuyAndSellStockIII {
	 ...
	 * 一个是当前到达第i天可以最多进行j次交易
	 * ，最好的利润是多少（global[i][j]），另一个是当前到达第i天，最多可进行j次交易，并且最后一次交易在当天卖出的最好的利润是多少
	 * （local[i][j]）。下面我们来看递推式，全局的比较简单，
	 ...
	 * 第二个量则是取local第i
	 * -1天j次交易，然后加上今天的差值（这里因为local[i-1][j]比如包含第i-1天卖出的交易，所以现在变成第i天卖出
	 * ，并不会增加交易次数，而且这里无论diff是不是大于0都一定要加上，因为否则就不满足local[i][j]必须在最后一天卖出的条件了）。
	 * 上面的算法中对于天数需要一次扫描
	 * ，而每次要对交易次数进行递推式求解，所以时间复杂度是O(n*k)，如果是最多进行两次交易，那么复杂度还是O(n)。空间上只需要维护当天数据皆可以
	 * ，所以是O(k)，当k=2，则是O(1)。代码如下：
	 */
